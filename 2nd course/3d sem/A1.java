...
public class A1 {
    ...
    public static void main(String[] args) {
        ...
        for (Pair<Integer, Integer> p : ans) {
            res.append(p.first).append(" ").append(p.second).append(System.lineSeparator());
        }
        System.out.println(res);
    }
    public static void dfs(int num, boolean[] mark, ArrayList<Integer>[] graph, List<Pair<Integer, Integer>> res) {
        mark[num] = true;
        for (int i : graph[num]) {
            if (!mark[i]) {
                res.add(new Pair<>(num, i));
                dfs(i, mark, graph, res);
            }
        }
    }
}